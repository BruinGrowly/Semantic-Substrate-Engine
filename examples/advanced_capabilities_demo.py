...
This script showcases the sophisticated systems that can be built upon the
foundational `DivineInvitationSemanticEngine`. It is not a core engine component,
but a powerful example of the engine's potential.
"""
...
class VocabularyManager:
    ...
    def _build_complete_vocabulary(self):
        ...
        for concept, dimensions in self.enhanced_vocab.items():
            if concept not in self.keyword_map:
                # For simplicity, we assign the concept itself to the first dimension in its list
                self.keyword_map[concept] = Dimension[dimensions[0].upper()] if isinstance(dimensions, list) else Dimension[dimensions.upper()]
            for word in dimensions:
                 if word not in self.keyword_map:
                    # This is imperfect, as it doesn't handle multi-word concepts well, but it's a start.
                    # A better system would be to have a proper graph, but for now, this will add more keywords.
                    # Let's map the dimension words back to their dimension
                    for dim_enum in Dimension:
                        if dim_enum.value == word:
                            self.keyword_map[word] = dim_enum
        ...
        for word in words:
            if word in self.keyword_map:
                ...
                if isinstance(dimension, Dimension):
                    ...
                    # Handle case where dimension is a string
                    counts[dimension] += 1
...
class GeopoliticalAnalyzer:
    ...
    def analyze_entity_posture(self, entity_name: str, entity_type: str,
        ...
        # This part of the logic is flawed, as it modifies the shared vocabulary.
        # A better approach would be to create a temporary, context-specific vocabulary.
        # For now, we will proceed with the current logic but acknowledge this flaw.
        type_vocab = self.vocab.enhanced_vocab.get(entity_type.lower(), [])
        # self.vocab.keyword_map.update(type_vocab) # This line is problematic
...
class UnifiedAnalysisEngine:
    ...
    def calculate_unified_score(self, coords_list: List[Coordinates]) -> Dict:
        ...
        semantic_analyzer = SemanticAnalyzer(self.vocab)
        concepts_for_analysis = ["justice", "fairness", "rights", "equality", "law"]  # Use known concepts
        semantic_result = semantic_analyzer.analyze_concept_cluster(concepts_for_analysis)

        # Check if vocabulary supports advanced analysis
        has_advanced_vocab = any(word in self.vocab.keyword_map for word in ['government', 'democracy', 'rights', 'law'])
...
def main():
    ...
    print(f"   Distance from Anchor: {unknown_result.distance_from_anchor:.3f}")
    ...
    print(f"   Concept Count: {harmony_result.concept_count}")
    print(f"   Confidence: {harmony_result.confidence:.3f}")
